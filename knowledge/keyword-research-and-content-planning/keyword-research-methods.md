---
type: Document
title: Manual and tool-based keyword research methods
...
## Method 3: the EMD secret-combo method

Best suited to niches built around Exact Match Domains (EMDs).

1. Search the keyword in Ahrefs or SEMrush and identify the top-ranking site.
2. Manually open the top results and copy every URL that matches the keyword
   exactly (EMD-based competitor sites).
3. Paste everything into one spreadsheet.
4. Repeat the same search with a second country/location filter (e.g. UK,
   US) and add those results too.
5. Remove duplicate sites from the combined list.
...
1. Enter the main competitor's domain into Site Explorer.
2. Select Top Pages, filtered by your target country or "All World".
3. Analyze which pages rank for which keywords.
4. Copy the top-performing URLs, check their structure in Google, and save
   everything to a spreadsheet.

**Finalization:** repeat Site Explorer against remaining competitors, export
keywords, and use Keyword Shitter again for any final manual extraction
before moving to article-writing prep.

## Tools used, at a glance

| Tool | Purpose |
| --- | --- |
| Google Instant | Quick keyword ideas |
| Keyword Helper | Long-tail keywords |
